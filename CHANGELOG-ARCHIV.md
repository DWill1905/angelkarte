# Changelog-Archiv (bis Version 1.0.0)

Vollständige, technisch detaillierte Historie aller Änderungen vor der Umstellung auf
[Semantic Versioning](https://semver.org/lang/de/). Jeder Eintrag war ursprünglich an
eine `beisszeit-shell-vN`-Cache-Version in `sw.js` gekoppelt (bis SW v140: `angelkarte-shell-vN`).

Der aktuelle, kurze **[CHANGELOG.md](CHANGELOG.md)** beginnt bei Version 1.0.0 und fasst
diese gesamte Historie in wenigen Zeilen zusammen. Diese Datei bleibt als Nachschlagewerk
für Details (genaue Ursache, betroffene Dateien, Testabdeckung) erhalten, wächst aber nicht
mehr weiter.
