...
export function kbHtml(){
  ...
  if(state.WX&&state.WX.wt!=null){
    ...
    if(wt<6) se='winter';
    else if(wt<12) se=(se==='herbst')?'herbst':'fruehjahr';
    else if(wt>=20) se='sommer';
  }
  ...
  if(state.WX){
    if(state.WX.trendVal<=-1.5) h+='<p style="margin-bottom:9px"><b>Druck fällt ('+de1(state.WX.trendVal)+' hPa/3h):</b> Fresslaune – aktiver und schneller führen, jetzt ans Wasser!</p>';
    else if(state.WX.trendVal>=1.5) h+='<p style="margin-bottom:9px"><b>Druck steigt:</b> nach der Front oft zäh – Köder verkleinern, langsamer führen, Tiefe suchen.</p>';
  }
  if(state.WX&&state.WX.wt!=null){
    if(state.WX.wt>=25) h+='<p style="margin-bottom:9px"><b>⚠ Wasser '+Math.round(state.WX.wt)+'°C:</b> Hitzestress – Entnahme statt C&amp;R, kurze Drills, tiefe/strömungsreiche Bereiche.</p>';
    else if(state.WX.wt<10) h+='<p style="margin-bottom:9px"><b>Kaltes Wasser ('+Math.round(state.WX.wt)+'°C):</b> Stoffwechsel runter – alles halb so schnell führen, Pausen verdoppeln.</p>';
    else if(state.WX.wt>=18&&state.WX.wt<23) h+='<p style="margin-bottom:9px"><b>Wasser '+Math.round(state.WX.wt)+'°C:</b> optimales Aktivitätsfenster für Zander &amp; Barsch.</p>';
  }
  const mn=new Date().getMonth()+1;
  if(mn>=4&&mn<=6) h+='<p style="margin-bottom:9px;color:var(--amber)"><b>Laichzeit-Rücksicht:</b> Flachbuchten und Schilfkanten sind jetzt Kinderstube – dort behutsam, Laichfische zurücksetzen, viele Arten sind ohnehin geschont.</p>';
...
byId('wxScore')?.addEventListener('click', () => { if (wxDlg) wxDlg.hidden = true; openScore(); });
...
if(satBtn){ satBtn.onclick=()=>{ toolsDlg.hidden=true; satToggle(); }; }
...
leadDlg.addEventListener('click',e=>{if(e.target===leadDlg)leadDlg.hidden=true;});
...
knotDlg.addEventListener('click',e=>{if(e.target===knotDlg)knotDlg.hidden=true;});

export const packDlg=byId('packDlg');
export async function packState(){ try{return JSON.parse((await store.get('pack:'+state.REGION.id)).value)||{};}catch(e){return {};} }
...
async function erlaubnisDatum(): Promise<string> {
  if(!state.REGION) return '';
  try{ return (await store.get('erlaubnis:'+state.REGION.id)).value||''; }catch(e){ return ''; }
}
...
export async function checkErlaubnisAblauf(): Promise<void> {
  const warn=byId('erlaubnisWarn');
  if(!warn||!state.REGION) return;
  const iso=await erlaubnisDatum();
  if(!iso){ warn.classList.remove('show'); warn.innerHTML=''; return; }
  const ablauf=new Date(iso+'T00:00:00');
  if(isNaN(ablauf.getTime())){ warn.classList.remove('show'); warn.innerHTML=''; return; }
...
export async function openPack(){
  ...
  if(erlEl) erlEl.onchange=async()=>{ await setErlaubnisDatum(erlEl.value); await checkErlaubnisAblauf(); };
...
function renderBite(){
  ...
  const lbl=byId('biteTagLabel'); if(lbl) lbl.textContent=tagLabel(biteTag, now);
  const tt=byId('biteTitel'); if(tt) tt.textContent = heute?'Beißzeiten heute':'Beißzeiten '+tagLabel(biteTag, now).replace(/^Morgen · /,'morgen, ');
  const pv=byId('bitePrev') as HTMLButtonElement|null; if(pv) pv.disabled = biteTag<=0;
  const nx=byId('biteNext') as HTMLButtonElement|null; if(nx) nx.disabled = biteTag>=MAX_TAG;
  ...
  if(!win.length){ h+='<p>Für diesen Tag lassen sich keine Fenster berechnen (Polartag/-nacht?).</p>'; }
  else {
...
byId('bitePrev').onclick=()=>{ if(biteTag>0){ biteTag--; renderBite(); } };
byId('biteNext').onclick=()=>{ if(biteTag<MAX_TAG){ biteTag++; renderBite(); } };
...
export async function openForecast(){
  ...
  if(count>0){
    icsBtn.onclick=()=>{
      ...
      a.href=url; a.download='beisszeit-woche.ics';
...
export function computeScore(){
  ...
  if(state.WX && typeof state.WX.trendVal==='number'){
    let pt,txt;
    if(state.WX.trendVal<=-1.5){pt=2;txt='Fallender Druck ('+de1(state.WX.trendVal)+' hPa/3h) – Fresslaune';}
    else if(state.WX.trendVal<1.5){pt=1;txt='Druck stabil ('+de1(state.WX.trendVal)+' hPa/3h) – neutral';}
    else{pt=0;txt='Druck steigt ('+de1(state.WX.trendVal)+' hPa/3h) – nach Front oft zäh';}
  ...
  if(wins.length){
    ...
    let pt,txt;
    if(inWin&&inWin.type==='major'){pt=2;txt='Major-Beißfenster läuft gerade ('+inWin.label+')';}
    else if(inWin){pt=1;txt='Minor-Beißfenster läuft ('+inWin.label+')';}
    else{
      const next=wins.filter(w=>w.from>now).sort((a,b)=>a.from-b.from)[0];
      if(next&&(next.from-now.getTime())<=90*60e3){pt=1;txt='Beißfenster in '+Math.round((next.from-now.getTime())/60e3)+' min ('+next.label+')';}
      else{pt=0;txt=next?'Nächstes Fenster erst '+hhmm(next.from)+' Uhr':'Kein Fenster in Reichweite';}
    }
    factors.push({name:'Beißfenster',pts:pt,max:2,txt});
  } else factors.push({name:'Beißfenster',pts:null,max:2,txt:'Sonnen-/Monddaten nicht verfügbar'});
  ...
  if(wt!=null){
    let pt,txt;
    if(wt>=12&&wt<=20){pt=2;txt='Wasser '+Math.round(wt)+'°C – optimales Raubfischfenster';}
    else if((wt>=8&&wt<12)||(wt>20&&wt<25)){pt=1;txt='Wasser '+Math.round(wt)+'°C – brauchbar';}
    else if(wt>=25){pt=0;txt='Wasser '+Math.round(wt)+'°C – Hitzestress, träge';}
    else{pt=0;txt='Wasser '+Math.round(wt)+'°C – Kälte-Lethargie';}
    factors.push({name:'Wassertemp.',pts:pt,max:2,txt});
  } else factors.push({name:'Wassertemp.',pts:null,max:2,txt:'Keine Wassertemperatur verfügbar'});
  ...
  if(istFluss){
    ...
    let pt,txt;
    if(hoch){pt=0;txt='Hochwasser ('+state.PEGEL.value+' cm) – schwer befischbar';}
    else if(starkSteigend){pt=1;txt='Pegel steigt stark (+'+state.PEGEL.trend+' cm/24h) – Fische umorientiert';}
    else{pt=2;txt='Pegel moderat ('+state.PEGEL.value+' cm) – gut befischbar';}
    factors.push({name:'Pegel',pts:pt,max:2,txt});
  } else factors.push({name:'Pegel',pts:null,max:2,txt:'Kein Pegel in Reichweite'});
  ...
  if(state.WX&&typeof state.WX.wind==='number'){
    let pt,txt;
    if(state.WX.wind>=5&&state.WX.wind<=20){pt=2;txt='Wind '+Math.round(state.WX.wind)+' km/h – Wellen bringen Aktivität';}
    else if(state.WX.wind<5){pt=1;txt='Wenig Wind ('+Math.round(state.WX.wind)+' km/h) – Wasser ruhig';}
    else if(state.WX.wind<35){pt=0.5;txt='Starker Wind ('+Math.round(state.WX.wind)+' km/h) – schwer zu werfen, Wellen';}
    else{pt=0;txt='Sturm ('+Math.round(state.WX.wind)+' km/h) – nicht befischbar';}
    factors.push({name:'Wind',pts:pt,max:2,txt});
  } else factors.push({name:'Wind',pts:null,max:2,txt:'Keine Winddaten'});
  ...
  if(sturm){
    if(pct!=null) pct=Math.min(pct,15);
...
export function scoreAmpel(pct){
  if(pct==null) return {label:'Zu wenig Daten',color:'var(--muted)',emoji:'○'};
  if(pct>=75) return {label:'Top',color:'var(--ok)',emoji:'●'};
  if(pct>=55) return {label:'Gut',color:'var(--dusk)',emoji:'●'};
  if(pct>=35) return {label:'Mäßig',color:'var(--amber)',emoji:'●'};
...
export function openScore(){
  ...
  if(r.sturm) h+='<div class="rate-sturm">⚠ Sturm – Angeln ist heute unverantwortlich. Der Wert unten ist gedeckelt.</div>';
  /* Kopf: große Ampel */
  ...
  if(r.knownCount<r.factors.length) h+='<div style="font-size:11px;color:var(--muted);margin-top:4px">'+r.knownCount+' von '+r.factors.length+' Faktoren mit Daten – lade Wetter/Pegel für ein volles Bild</div>';
  h+='</div>';
  /* Faktoren */
...
scoreDlg.addEventListener('click',e=>{if(e.target===scoreDlg)scoreDlg.hidden=true;});
...
kbDlg.addEventListener('click',e=>{if(e.target===kbDlg)kbDlg.hidden=true;});
...
export function enhanceDetail(wrap: HTMLElement | null): void {
  if(!wrap) return;
  wrap.querySelectorAll('.tackle > summary, .pop-details > summary').forEach((sum: Element)=>{
    sum.addEventListener('click',(ev: Event)=>{
  ...
  const wtEl=wrap.querySelector('[data-wt]') as HTMLElement|null;
  if(wtEl){
  ...
  const tEl=wrap.querySelector('.pop-btn.trip') as HTMLElement|null;
  if(tEl) setTripBtn(tEl,inTrip(tEl.dataset.spot));
  const wEl=wrap.querySelector('[data-wind]') as HTMLElement|null;
  if(wEl){
  ...
  const el=wrap.querySelector('[data-dist]') as HTMLElement|null;
  if(el){
  ...
  const notizEl=wrap.querySelector('.notiz-ta') as HTMLTextAreaElement|null;
  if(notizEl){
    const name=notizEl.dataset.notizSpot||'';
    ladeNotiz(name).then(txt=>{ if(notizEl.value==='') notizEl.value=txt; }); /* Detail evtl. laengst zu/neu geoeffnet */
    notizEl.onblur=()=>{ speichereNotiz(name,notizEl.value); };
  }
}

/* Vollbild-Panel für Tackle / Gewässer & Methode. */
function oeffneDetail(titel: string, html: string): void {
  const sheet=byId('detailSheet') as HTMLElement|null;
  const t=byId('detailTitle'); const b=byId('detailBody');
  if(!sheet||!b) return;
...
(function(){
  ...
  const c=byId('detailClose') as HTMLElement|null;
  if(c) c.onclick=zu;
  sheet.addEventListener('click',ev=>{ if(ev.target===sheet) zu(); });
  document.addEventListener('keydown',ev=>{ if((ev as KeyboardEvent).key==='Escape' && !sheet.hidden) zu(); });
})();
