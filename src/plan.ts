...
function renderZeitBody(liste: Kandidat[], zeitpunkt: Date, offset: number): string {
  ...
  const zeile = (k: Kandidat, i: number) =>
    '<div class="plan-rang"><span class="n">' + (i + 1) + '.</span>'
    + '<span class="ort">' + esc(k.ort) + ' · ' + esc(k.art) + '</span>'
    + '<span class="proz">' + k.basis + ' %</span></div>';
  let h = '';
  if (sturm) h += '<div class="rate-sturm">⚠ Sturm – Angeln ist zu diesem Zeitpunkt unverantwortlich. Die Chancen unten sind entsprechend gedeckelt.</div>';
  ...
  if (!top.length) return h + leerText();
...
function goToKandidat(): void {
  ...
  if (letzterKandidat && state.map) {
    ...
    if (letzterKandidat.hotspot) selectHotspot(letzterKandidat.spot, letzterKandidat.hotspot);
    else selectSpot(letzterKandidat.spot);
  }
}
...
function rerenderPlan(): void {
  ...
  if (planZeit) {
    ...
    if (go) { go.hidden = !liste.length; go.onclick = goToKandidat; }
    return;
  }
...
export function openPlan(): void {
  ...
  planTag = 0;
  planZeit = '';
  const zeitEl = byId('planZeit') as HTMLInputElement | null;
  if (zeitEl) zeitEl.value = '';
  const resetEl = byId('planZeitReset') as HTMLButtonElement | null;
...
if (planDlg) {
  ...
  const zeitEl = byId('planZeit') as HTMLInputElement | null;
  const resetEl = byId('planZeitReset') as HTMLButtonElement | null;
  if (zeitEl) zeitEl.onchange = () => {
    planZeit = zeitEl.value;
    if (resetEl) resetEl.hidden = !planZeit;
    rerenderPlan();
  };
