...
export const WT_OPT: Record<string, [number, number]> = {
  Hecht: [4, 18],
  Zander: [10, 22],
  Barsch: [8, 22],
  Wels: [18, 26],
...
function ruteAus(arten: string[], w: Wasser): string {
  ...
  if (!arten.length) return 'Ohne Zielfischangabe nicht ableitbar';
...
function farbenAus(w: Wasser, trueb?: boolean): Farben {
...
function koederVon(arten: string[], keinAnfuettern?: boolean): string {
  const erste = arten.find((a) => PROFILE[a]?.koeder);
  if (!erste) return 'Nach Zielfisch wählen – siehe Köderberater im Werkzeuge-Menü';
  if (keinAnfuettern && FUTTERKORB_ARTEN.has(erste)) return OHNE_ANFUETTERN;
