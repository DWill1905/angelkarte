...
export function mySpotObj(m: any): Spot {
  return {name:esc(m.name),nr:m.tiefe?('Tiefe '+m.tiefe+' m'):'Eigener Spot',cat:'eigen' as const,arten:[],lat:m.lat,lng:m.lng,
...
export function openMyDlg(){
  ...
  inputById('myName').value='';inputById('myTipp').value='';inputById('myTiefe').value='';myDlg.hidden=false;inputById('myName').focus();
}
...
export async function saveMySpot(){
  if(!state.REGION||(!myPending&&myEditId==null)) return closeMyDlg();
  ...
  if(tiefeEl.value&&!tiefeEl.checkValidity()){ tiefeEl.reportValidity(); return; }
...
window.delMySpot=async function(id){
  ...
  deselectSpot(); buildChips(); applyFilters();
};
