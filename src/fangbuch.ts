...
export async function fbPersist(){
  try{ await store.set('fangbuch',JSON.stringify(state.fbMem)); }
  catch(e){ state.persistent=false; }
  fbRender();
}
export function fbCsv(){
  if(!state.fbMem.length) return;
  ...
  a.href=URL.createObjectURL(blob); a.download='fangbuch.csv';
  document.body.appendChild(a); a.click(); a.remove();
  setTimeout(()=>URL.revokeObjectURL(a.href),1000);
}
export function topCount(arr: (string|null|undefined)[]): [string, number][] {
  const m={}; arr.forEach(k=>{if(k)m[k]=(m[k]||0)+1;});
  return (Object.entries(m) as [string, number][]).sort((a,b)=>b[1]-a[1]);
}
export function bars(entries: [string, number][], total: number): string {
  return entries.slice(0,4).map(([k,n])=>
...
export function fbInsights(){
  ...
  if(koeder.length){ h+='<b>Fängigste Köder</b>'+bars(koeder,state.fbMem.length); }
  const zeiten=topCount(state.fbMem.map(e=>{ if(!e.ctx||!e.ctx.zeit)return null; const hh=+e.ctx.zeit.slice(0,2); return hh<6?'Nacht (0–6)':hh<10?'früh (6–10)':hh<16?'Tag (10–16)':hh<21?'Abend (16–21)':'Nacht (21–24)'; }).filter(Boolean));
  if(zeiten.length){ h+='<b style="display:block;margin-top:8px">Beste Tageszeiten</b>'+bars(zeiten,state.fbMem.filter(e=>e.ctx&&e.ctx.zeit).length); }
  ...
  const rekorde: Record<string, {laenge:number;datum:string;spot:string}> = {};
  state.fbMem.forEach(e=>{
...
export async function fbRestore(file){
  ...
  let neu=0;
  gueltig.forEach(e=>{
    if(vorhanden.has(fangKey(e))) return;
    ...
    if(typeof e.id!=='number'||!Number.isFinite(e.id)||idsBelegt.has(e.id)) e.id=uid();
    idsBelegt.add(e.id);
    state.fbMem.push(e); vorhanden.add(fangKey(e)); neu++;
  });
...
export function fbTools(){
  ...
  imp.onclick=()=>inputById('fbFile').click(); t.appendChild(imp);
}
...
export function fbSortiert(): Fang[] {
...
export function fangTeilenText(e: Fang): string {
  let kopf='🎣 '+e.fisch+(e.laenge?', '+e.laenge+' cm':'');
  const zeilen=[kopf];
  const orts=[e.spot,e.datum].filter(Boolean);
  if(orts.length) zeilen.push(orts.join(' · '));
  const kontext: string[]=[];
  if(e.ctx){
    if(e.ctx.wt!=null) kontext.push('Wasser '+e.ctx.wt+'°C');
    if(typeof e.ctx.trend==='number'&&e.ctx.trend<=-1.5) kontext.push('fallender Luftdruck');
    if(e.ctx.fenster) kontext.push(e.ctx.fenster==='major'?'Major-Fenster':'Minor-Fenster');
  }
...
async function teileFang(id: number): Promise<void> {
  ...
  if(typeof nav.share==='function'){
    try{ await nav.share({text}); }catch(e){ /* Nutzer hat abgebrochen – kein Fehler */ }
  } else if(nav.clipboard&&typeof nav.clipboard.writeText==='function'){
    try{ await nav.clipboard.writeText(text); if(st) st.textContent='📋 In die Zwischenablage kopiert'; }
    catch(e){ if(st) st.textContent='❌ Konnte nicht in die Zwischenablage kopieren'; }
  }
}
...
export function fbRender(){
  ...
  fbSortiert().forEach(e=>{
    ...
    if(teilenVerfuegbar) qs<HTMLElement>('.fb-share', d).onclick=ev=>{
    ...
    qs<HTMLElement>('.fb-edit', d).onclick=async ev=>{
      ...
      if(lf.scrollIntoView) lf.scrollIntoView({behavior:'smooth',block:'center'});
      const st=byId('fbStatus');
      if(st){ st.textContent='✎ Eintrag geladen – anpassen und erneut speichern'; }
    };
    list.appendChild(d);
  });
}
export function checkFang(){
  ...
  if(override) msgs.push('ℹ An diesem Gewässer gilt eine abweichende Vereinsregel für '+fisch+' – strenger geprüft als die regionsweiten Werte im Regeln-Tab.');
  if(!sc){
  ...
  if(sc){
    ...
    if(!isNaN(len)){
      ...
      else if(m.max&&len>m.max){bad=true;msgs.push('⛔ Über dem Entnahmefenster: '+len+' cm > '+m.max+' cm – zurücksetzen!');}
      else if(m.min){msgs.push('✓ Maß in Ordnung ('+(m.max?m.min+'–'+m.max:'ab '+m.min)+' cm).');}
    }
  }
  ...
  if(state.REGION.id==='mecklenburg'&&['Hecht','Zander','Aal','Karpfen'].includes(fisch)){
    ...
    if(n>=3){bad=true;msgs.push('⛔ Tageslimit erreicht: '+n+'/3 entnommene Raubfische (Hecht/Zander/Aal/Karpfen).');}
    else msgs.push('Entnahme-Zähler heute: '+n+'/3 (Hecht/Zander/Aal/Karpfen).');
  }
  if(state.REGION.id==='erzgebirge'&&fisch==='Barsch'){
    ...
    if(n>=10){bad=true;msgs.push('⛔ Barsch-Tageslimit erreicht ('+n+'/10 entnommen).');}
    else msgs.push('Barsch-Entnahme heute: '+n+'/10.');
    if(ueber30>=5){bad=true;msgs.push('⛔ Teilquote über 30 cm erreicht ('+ueber30+'/5) – weitere nur unter 30 cm entnehmen.');}
    else if(ueber30>0) msgs.push('Davon über 30 cm: '+ueber30+'/5.');
  }
  if(state.REGION.id==='main'&&['Barsch','Hecht','Zander'].includes(fisch)){
    ...
    const vor7Tagen=new Date(); vor7Tagen.setDate(vor7Tagen.getDate()-6); vor7Tagen.setHours(0,0,0,0);
    const wocheN=state.fbMem.filter(e=>{
      if(!e.entnommen||!raub.includes(e.fisch)) return false;
      const d=parseFangDatum(e.datum);
      return !!d&&d>=vor7Tagen;
    }).length;
    if(heuteN>=3){bad=true;msgs.push('⛔ Tageslimit erreicht: '+heuteN+'/3 entnommene Raubfische (Barsch/Hecht/Zander).');}
    else msgs.push('Entnahme heute: '+heuteN+'/3 Raubfische (Barsch/Hecht/Zander).');
...
byId('fbSave').onclick=async ()=>{
  ...
  if(!fischAndEl.checkValidity()){ fischAndEl.reportValidity(); return; }
  if(!spotAndEl.checkValidity()){ spotAndEl.reportValidity(); return; }
...
export function prefillFang(spotName){
  ...
  [...fbSpotSel.options].forEach(o=>{if(o.textContent===spotName)fbSpotSel.value=spotName;});
}
...
document.addEventListener('visibilitychange',()=>{ if(!document.hidden){state.wxKey='';loadWeather();} });
setInterval(()=>{ if(!document.hidden){state.wxKey='';loadWeather();} },20*60*1000);
...
if(fbFileEl){
  fbFileEl.onchange=async (e: Event)=>{
