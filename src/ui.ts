...
(function initDialogFocusReturn(): void {
  const trigger = new WeakMap<Element, HTMLElement>();
  qsa<HTMLElement>('.mydlg-wrap').forEach((dlg) => {
    new MutationObserver(() => {
      if (!dlg.hidden) {
        const active = document.activeElement;
        if (active instanceof HTMLElement && active !== document.body) trigger.set(dlg, active);
      } else {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    const open = qsa<HTMLElement>('.mydlg-wrap').find((d) => !d.hidden);
