...
async function ladeTagesmittel(ctr) {
    try {
...
export function wtSchaetzung(wasser) {
    const d = state.WXD;
    if (!d || !Array.isArray(d.mean))
        return null;
    const temps = d.mean.filter((v) => typeof v === 'number');
    if (temps.length < 10)
        return null;
    ...
    const ema = (arr) => { let e = arr[0]; for (const t of arr)
        e = e + alpha * (t - e); return e; };
    ...
    const clamp = (v) => Math.max(1, Math.min(30, v));
...
export async function loadWeather() {
    ...
    try {
        ...
        if (state.PEGEL && state.PEGEL.key !== key)
            state.PEGEL = null;
        ...
        if (!(state.PEGEL && state.PEGEL.wt != null)) {
            ...
            if (est)
                el.innerHTML += ' · Wasser ≈' + Math.round(est.wert) + '°C (geschätzt aus Lufttemperatur)';
        }
    }
    catch (e) {
...
export function wxAt(d) {
    const now = new Date();
    if (Math.abs(d.getTime() - now.getTime()) < 45 * 60e3)
        return state.WX;
    const h = state.WXH;
    if (!h || !h.time)
        return state.WX;
    ...
    if (temp == null || wind == null || press == null)
        return null;
    const dirs = ['N', 'NO', 'O', 'SO', 'S', 'SW', 'W', 'NW'];
...
export function checkStorm(d, c) {
    ...
    if ([95, 96, 99].includes(codeNow)) {
        msg = '⛈ GEWITTER JETZT – sofort raus aus dem Wasser, Rute ablegen, Schutz suchen!';
    }
...
export async function loadPegel(ctr, el, key = '') {
    try {
        ...
        if (dist != null && dist > 20)
            txt += ' – Station weit entfernt, nur grober Anhalt';
        /* Trend über 24 h */
        try {
            ...
            if (Array.isArray(hist) && hist.length > 1) {
                ...
                if (Math.abs(d) >= 5)
                    txt += ' (' + (d > 0 ? '+' : '') + d + ' cm/24h)';
                if (d >= 25)
                    txt += ' – steigt &amp; trübt: Räuber ans Ufer gedrückt!';
            }
        }
        catch (e) { }
        ...
        if (wtSt)
            try {
                ...
                if (wt && typeof wt.value !== 'undefined') {
                    ...
                    if (wtDist != null)
                        state.PEGEL.wtDist = wtDist;
                    if (state.WX)
                        state.WX.wt = wt.value;
                    ...
                    try {
                        ...
                        if (vals.length >= 12) {
                            ...
                            if (Math.abs(dTrend) >= 1)
                                txt += ' (' + (dTrend > 0 ? '+' : '') + dTrend + '°C/3T' + (dTrend <= -1.5 ? ' – Kälteeinbruch' : dTrend >= 1.5 ? ' – Erwärmung' : '') + ')';
                        }
                    }
                    catch (e) { }
                }
        ...
        if (state.REGION && state.REGION.pegel && m.value >= state.REGION.pegel.warnAb)
            txt += ' · ⚠ ' + state.REGION.pegel.text;
        el.innerHTML += txt;
    }
    catch (e) { }
}
