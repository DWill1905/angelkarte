...
export async function toggleTrip(name){
  ...
  else state.trip.push({region:rid,name,notiz:''});
  await tripSave();
  updateTripBadge();
  /* offenes Popup aktualisieren */
  /* offenes Popup aktualisieren – ohne CSS.escape (nicht überall verfügbar) */
  document.querySelectorAll('.pop-btn.trip').forEach(btn=>{
    if(btn.dataset.spot===name) setTripBtn(btn,inTrip(name));
  });
}
...
export function updateTripBadge(){
  const b=document.getElementById('tripCount');
  if(b) b.textContent = state.trip.length?String(state.trip.length):'';
}

export const tripDlg=document.getElementById('tripDlg');
export function openTrip(){
  ...
  body.querySelectorAll('.trip-del').forEach(b=>{
    b.onclick=async()=>{
      ...
      await tripSave(); updateTripBadge(); openTrip();
    };
  });
  const clr=document.getElementById('tripClear');
  if(clr) clr.onclick=async()=>{ state.trip=[]; await tripSave(); updateTripBadge(); openTrip(); };
  tripDlg.hidden=false;
}
if(tripDlg){
  document.getElementById('tripClose').onclick=()=>{tripDlg.hidden=true;};
  tripDlg.addEventListener('click',e=>{if(e.target===tripDlg)tripDlg.hidden=true;});
}
