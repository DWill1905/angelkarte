...
function zeitBewertung(lat, lng, jetzt, daemmerungsfisch, nachtfisch = false) {
    ...
    const fenster = (solunar(lat, lng, jetzt) || []).map((f) => ({ ...f, from: new Date(f.from), to: new Date(f.to) }));
    ...
    const laeuft = fenster.find((f) => t >= f.from.getTime() && t <= f.to.getTime());
    if (laeuft) {
    ...
    const kommt = fenster.filter((f) => f.from.getTime() > t).sort((a, b) => a.from - b.from)[0];
...
function lichtBewertung(lat, lng, jetzt, wx, art, p, trueb) {
    ...
    if (p?.licht === 'scheu') {
        if (wenigLicht)
            return { status: 'ja', text: `Gedämpftes Licht (bedeckt/Welle/Regen) – ideal für den lichtscheuen ${art}`, erreicht: G, moeglich: G };
        if (sonnig && flau)
            return { status: 'nein', text: `Grelles Licht auf spiegelglattem Wasser – ${art} ist lichtscheu und steht tief`, erreicht: 0.3, moeglich: G };
        return { status: 'ja', text: `Wechselndes Licht – ${art} jagt, wenn Wolken oder Wellen es brechen`, erreicht: 1, moeglich: G };
    }
    if (p?.licht === 'sicht') {
        if (regen && bedeckt)
            return { status: 'nein', text: `Trüb und dunkel – der Sichträuber ${art} findet die Beute schlechter`, erreicht: 0.6, moeglich: G };
        if ((sonnig || code === 2) && chop)
            return { status: 'ja', text: `Gutes Licht mit Kräuselung – perfekt für den Sichträuber ${art}`, erreicht: G, moeglich: G };
        if (sonnig && flau)
            return { status: 'nein', text: `Spiegelglatt und grell – Sichträuber wie ${art} werden scheu`, erreicht: 0.6, moeglich: G };
        return { status: 'ja', text: `Solides Licht für den Sichträuber ${art}`, erreicht: 1.1, moeglich: G };
    }
    if (p?.warm || p?.truebung) {
        if (bedeckt || regen || chop)
            return { status: 'ja', text: `Bedeckt, Regen oder Welle – angenehm für ${art}`, erreicht: G, moeglich: G };
...
export function bewerteSpot(s, art, jetzt = new Date(), hotspot = null) {
    const gruende = [];
    ...
    const add = (status, text, erreicht, moeglich) => gruende.push({ status, text, erreicht, moeglich });
    ...
    if (!wx) {
        ...
        if (wx.wind >= 35)
            add('nein', `Sturm (${Math.round(wx.wind)} km/h) – gefährlich und meist unfischbar`, 0, 1.5);
        else if (auf === true && wx.wind >= 6)
            add('ja', `Wind aus ${wx.dir} drückt aufs Ufer – Nahrung treibt heran`, 1.5, 1.5);
        else if (auf === false && wx.wind >= 12)
            add('nein', `Uferabschnitt liegt im Windschatten (Wind aus ${wx.dir})`, 0.4, 1.5);
        else if (wx.wind >= 8 && wx.wind < 25)
            add('ja', `Leichte Welle (${Math.round(wx.wind)} km/h) – gut, bricht das Licht`, 1.2, 1.5);
        else if (wx.wind < 4)
            add('nein', 'Windstille – blankes Wasser, Fische scheu', 0.5, 1.5);
        else
            add('ja', `Wind ${Math.round(wx.wind)} km/h aus ${wx.dir}`, 1, 1.5);
    }
    ...
    if (hotspot) {
        if (hotspotAktiv(hotspot, monat))
            add('ja', `„${hotspot.name}" ist jetzt in Saison`, 1, 1);
        else
            add('nein', `„${hotspot.name}" ist außerhalb seiner Saison (${hotspot.saison})`, 0, 1);
    }
...
export function sterneAus(prozent, gesperrt = false) {
    if (gesperrt)
        return prozent >= 15 ? 1 : 0;
    if (prozent >= 85)
        return 5;
    if (prozent >= 72)
        return 4;
...
function monatName(m) { return MON[m - 1] || ''; }
/** Bewertet alle sinnvollen Zielarten eines Spots, beste zuerst. */
export function bewerteAlle(s, jetzt = new Date(), hotspot = null) {
...
export function ratingHtml(s, hotspot = null) {
    ...
    const zeilen = top.map((b, i) => {
        ...
        const gruende = bekannt.map((g) => `<div class="rate-g ${g.status}"><span class="rate-ik">${IKON[g.status]}</span> ${esc(g.text)}</div>`).join('');
        const meta = [];
        if (b.bewertet < b.gesamt)
            meta.push(`Datenbasis ${b.bewertet}/${b.gesamt} Signale${b.konfidenz < 0.8 ? ' – dünn, Wert zur Mitte gedämpft' : ''}`);
        if (fehlt)
            meta.push(`${fehlt} Signal${fehlt > 1 ? 'e' : ''} fehlen gerade (offline / kein Pegel / keine Historie)`);
