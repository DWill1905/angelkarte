...
export async function initRegions(){
  try{
    ...
    const loaded=await Promise.all(idx.map(f=>fetch('data/'+f,{cache:'no-cache'}).then(r=>{if(!r.ok)throw 0;return r.json();})));
    state.REGIONS=loaded.filter(r=>r&&r.id&&Array.isArray(r.spots));
    if(!state.REGIONS.length) throw 0;
  }catch(e){ state.REGIONS=REGIONS_EMBEDDED; }
  ...
  let saved=null; try{saved=(await store.get('region')).value;}catch(e){}
  const start=state.REGIONS.find(r=>r.id===saved)||state.REGIONS[0];
  sel.value=start.id;
  sel.onchange=async()=>{loadRegion(state.REGIONS.find(r=>r.id===sel.value));try{await store.set('region',sel.value);}catch(e){}};
  loadRegion(start);
}
...
export async function loadRegion(r){
  ...
  if(state.REGION) state.SPOTS.forEach(sp=>{
    if(sp.marker&&state.map.hasLayer(sp.marker))state.map.removeLayer(sp.marker);
    (sp.hotMarkers||[]).forEach(m=>{if(state.map.hasLayer(m))state.map.removeLayer(m);});
  });
  ...
  document.getElementById('footRegion').innerHTML=r.fusszeile||'';
  document.getElementById('appTitle').innerHTML='Angelrevier '+(r.kurz||r.name)+' <span class="tilde">~</span>';
