...
export let selectedSpot = null;
export let selectedHotspot = null;
function detailBlockHtml() {
    if (!selectedSpot)
        return '';
...
export function renderList() {
    ...
    if (selectedSpot) {
        ...
        const x = byId('spotDetailClose');
        if (x)
            x.onclick = deselectSpot;
    }
}
renderList();
export const sheet = byId('sheet'), handle = byId('sheetHandle');
export function setSheetOpen(open) { sheet.classList.toggle('collapsed', !open); }
...
export function selectHotspot(parent, h) {
    selectedSpot = parent;
    selectedHotspot = h;
    setSheetOpen(true);
    setDetailOffen(true);
...
function setDetailOffen(offen) { document.body.classList.toggle('detail-offen', offen); }
...
function kombiniertesSignal(a, b) {
    const ctrl = new AbortController();
    const abbrechen = () => ctrl.abort();
    if (a.aborted || b.aborted)
        ctrl.abort();
    else {
        a.addEventListener('abort', abbrechen, { once: true });
        b.addEventListener('abort', abbrechen, { once: true });
    }
    return ctrl.signal;
}
export async function cacheViewport(prog, signal) {
    ...
    for (const url of urls) {
        ...
        try {
            ...
            if (hit) {
                ...
                const r = await fetch(url, { mode: 'no-cors', signal: fetchSignal });
...
export async function openOffline() {
    ...
    if (mySignal.aborted)
        return; // Dialog wurde inzwischen geschlossen - keine Meldung mehr reinschreiben
    if (res.noSupport) {
...
if (offDlg) {
    const offAbbrechen = () => { offDlg.hidden = true; if (offlineAbort)
        offlineAbort.abort(); };
