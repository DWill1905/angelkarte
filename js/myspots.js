...
export function openMyDlg(){document.getElementById('myName').value='';document.getElementById('myTipp').value='';document.getElementById('myTiefe').value='';myDlg.hidden=false;document.getElementById('myName').focus();}
export function closeMyDlg(){myDlg.hidden=true;myPending=null;}
export async function saveMySpot(){
  if(!myPending||!state.REGION) return closeMyDlg();
  const m={id:uid(),
    name:document.getElementById('myName').value.trim()||'Eigener Spot',
    tipp:document.getElementById('myTipp').value.trim(),
    tiefe:(()=>{const t=parseFloat(document.getElementById('myTiefe').value);return(!isNaN(t)&&t>0&&t<=80)?t:null;})(),
    lat:myPending[0],lng:myPending[1]};
  const list=await loadMySpots(state.REGION.id); list.push(m);
  try{await store.set('myspots:'+state.REGION.id,JSON.stringify(list));}catch(e){}
...
document.getElementById('mySave').onclick=saveMySpot;
document.getElementById('myCancel').onclick=closeMyDlg;
...
mapC.addEventListener('touchstart',ev=>{
  if(ev.touches.length!==1||!state.REGION) return;
  lpMoved=false; lpStart=ev.touches[0];
  lpTimer=setTimeout(()=>{
    if(lpMoved) return;
    const pt=state.map.mouseEventToLatLng(lpStart);
    myPending=[pt.lat,pt.lng]; openMyDlg();
...
mapC.addEventListener('touchmove',ev=>{
  ...
  if(Math.abs(t.clientX-lpStart.clientX)>12||Math.abs(t.clientY-lpStart.clientY)>12){ lpMoved=true; clearTimeout(lpTimer); }
},{passive:true});
